...
- 遗嘱消息(推送流异常断开且宽限期内未重连时发布;宽限期支持会话级/流级设置,缺省用 Broker 预定义值。
  `retain` 遗嘱仅在 `clean_start=false` 会话上驻留为保留消息,会话恢复在线或销毁时收回并广播清除事件)
- SSE 心跳探测(周期推送 `: ping`,写失败即判离线;间隔支持流级/会话级/Broker 级设置)
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/sessions` | 创建会话 `{client_id?, clean_start?, will?, will_grace_period?, heartbeat_interval?}` → `{session_id, resumed}` |
| GET | `/sessions` | 列出会话 |
...
| GET | `/sessions/{id}/stream?will_grace_period=&heartbeat_interval=` | 打开 SSE 推送流(订阅表先经 HTTP 维护);查询参数可覆盖遗嘱宽限期与心跳间隔 |
...
- 慢消费者保护:每会话流缓冲默认 1(`WithStreamBuffer`),缓冲满则丢弃
